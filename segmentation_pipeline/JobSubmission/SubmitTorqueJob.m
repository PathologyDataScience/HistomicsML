function [status result] = SubmitTorqueJob(jobstring, jobid, mem)
...
%add command to CD to current working directory
...
if(nargin == 1) %submit job without -N
    [status result] = system(['qsub -V -k o -j oe -q matlab job.sh']); 
else %submit job with -N option
    if(~isletter(jobid(1))) %fix invalid job ids
        jobid = ['.' jobid];
    end
       
    [status result] = system(['qsub -V -k o -j oe -q matlab -N ' jobid ' job.sh']);
end
